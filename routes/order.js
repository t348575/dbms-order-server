...
router.get('/getOrders', authenticateJWT, (req, res) => {
    ...
    const onlyRecent = parseInt(req.body.recent, 10) || true;
    if (onlyRecent) {
        const today = new Date();
        today.setMonth(today.getMonth() - 6);
        localPool.query('SELECT ord_id, cust_id, ord_prod, ord_date, ord_summ, state FROM orders WHERE ord_date >= ?', [toMysqlFormat(today)], (err, results) => {
            if (err) {
                res.status(500).end();
            } else {
